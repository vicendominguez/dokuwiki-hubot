<?php
/**
 * DokuWiki Plugin hubot (Action Component)
 *
...
class action_plugin_hubot extends DokuWiki_Action_Plugin {
    ...
    function handle_action_act_preprocess(&$event, $param) {
      ...
      */  
        ...
        /* Namespace filter */
        $ns = $this->getConf('hubot_namespaces');
        if (!empty($ns)) {
        ...
        $hubotmsg = 'Wiki rlz!! ' . $fullname . ' ' .$this->getLang('hubot_message'). ' ' . $this->urlize();
        ...
        $data = array(
            "room"    => $this->getConf('conversation_id'),
            "message" => $hubotmsg
        );
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->getConf('hubot_url'));
        ...
        if ($result === false) {
            error_log("Error notifying hubot: " . curl_error($ch));
        }
    }
